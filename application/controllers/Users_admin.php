...
class Users_admin extends MY_Controller
{
	...
	{
		$row = $this->Users_model->get_detail_users($id);
		if ($row) {
			$data = array(
				'id' => $row->id,
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('users_admin/update_action'),
				'nama' => $row->nama,
				'hobi' => $row->hobi,
				'pekerjaan' => $row->pekerjaan,
				'deskripsi_diri' => $row->deskripsi_diri,
	...
	public function update_status()
	{
		...
		$row = $this->Users_model->get_detail_users($id_user);
		
		if ($row->active == 1) {
			$activeUpdate = 0;
		} elseif($row->active == 0 || $row->active == null) {
			$activeUpdate = 1;
		}
		$data = [
			'active' => $activeUpdate,
		];
		var_dump($row,$activeUpdate,$data);
		$this->Users_model->update($id_user, $data);
		$this->session->set_flashdata('message', 'Update Record Success');
		redirect(site_url('users_admin'));
	}
